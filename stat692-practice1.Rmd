...
After removing records with missing values, the dataset contains N penguins across three species: X Adelie, Y Chinstrap, and Z Gentoo.
...
```{r explore_data, echo=FALSE, fig.cap="Bill length vs. flipper length for known penguin species. The red star marks the unknown specimen (bill length = 45mm, flipper length = 220mm)."}
...
Multinomial logistic regression was chosen because the outcome has three unordered categories (Adelie, Chinstrap, Gentoo) and the predictors are continuous. A simpler approach such as nearest-centroid distance was considered but rejected because it does not provide probability estimates or account for the spread within each species cluster.
...
The model assigned an `r round(probabilities["Gentoo"]*100, 1)` probability to Gentoo, compared to `r round(probabilities["Chinstrap"]*100, 1)` for Chinstrap and `r round(probabilities["Adelie"]*100, 1)` for Adelie, making Gentoo the clear frontrunner
